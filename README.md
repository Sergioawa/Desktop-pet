# Desktop-pet
一个可自定义的桌面宠物程序。（使用 AI 工具辅助编程。）
A customizable desktop pet program. (Using AI tools to assist programming.)

欢迎使用这款万叶的桌面宠物程序。本程序使用Python开发，有以下功能：
1.支持添加桌宠的图片（适配性最好的是png格式的资源，动图gif的显示还有bug）
2.支持添加以及手动删除桌宠的语音（可以用mp3、wav文件）
3.支持更改图片切换的时间
4.本程序还在早期开发测试版本很多功能还未完善（懂得都懂，嘻嘻），有好的想法和程序的问题欢迎提交你的修改代码至仓库。
另外，理论上来说，只要你把img、voice里面的文件按照我说的文件标准换成自己喜欢的角色图片和语音，本程序就能成为你的专属桌宠啦。
Welcome to this desktop pet program featuring Kazuha. This program is developed in Python and has the following features:
1. Supports adding images for the desktop pet (png format resources are the most compatible; animated gif display still has bugs).
2. Supports adding and manually deleting the desktop pet's voice (mp3 and wav files can be used).
3. Supports changing the image switching time.
4. This program is still in the early development testing phase(Those who understand, understand. Hehe.), and many features are not yet complete. If you have good ideas or find issues with the program, you are welcome to submit your modified code to the repository.
Additionally, in theory, as long as you replace the files in the img and voice folders following the file standards I mentioned with your favorite character's images and voices, this program can become your very own desktop pet.
